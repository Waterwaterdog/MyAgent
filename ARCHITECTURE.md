...
本项目的核心设计遵循“模块化架构”和“可插拔组件”的原则，旨在实现一个清晰、可扩展、易于维护的 Coding Agent。架构的核心思想是将 Agent 的不同功能关注点分离到独立的、可替换的模块中。
...
- **Agent (`agent.py`)**: 作为顶层容器，负责初始化所有必要的组件（如 LLM 客户端、内存管理器等），并组装 `Agent Runtime`。它本身不包含复杂的业务逻辑，只提供一个统一的入口点 `run()`。

- **Agent Runtime (`runtime.py`)**: 这是 Agent 的核心执行引擎。它包含了主循环（Agent Loop）的全部逻辑，负责接收用户任务、管理规划与执行流程、调用大语言模型、处理工具调用、执行防死循环机制等。通过将执行逻辑集中到 `Runtime`，我们实现了“大脑”与“身体”的分离。

- **Planner / ReAct (`planning/` & `runtime.py`)**: 规划与推理模块。`Planner` 负责将宏观任务分解为步骤，而 ReAct 逻辑则在 `Runtime` 的循环中实现，负责动态的“思考-行动”循环。

- **Context / Memory Manager (`core/context.py`, `core/memory.py`)**: 上下文与记忆管理模块。`Context` 负责维护与 LLM 交互的消息历史和 Token 预算，而 `Memory` 则管理着短期、中期、长期的知识。

- **Model Client (`core/model_client.py`)**: 大语言模型客户端，封装了与 LLM API 的所有交互。这是一个可替换的组件，可以方便地切换到不同的模型提供商。

- **Tool System (`tools/`)**: 工具系统，包括 `Tool Registry`（工具注册表）和 `Tool Factory`。Agent Runtime 通过 `Tool Registry` 来发现和执行工具，而不需要知道任何工具的具体实现。这使得工具可以作为独立的插件进行开发和添加。

- **Trace System (`tracing/`)**: 全链路追踪系统，负责记录 Agent 执行过程中的所有关键事件，用于调试和分析。

- **Error System (`core/error.py`)**: 标准化的错误处理系统，定义了结构化的错误对象，使 Agent 能够理解并响应错误。

## 解耦原则

该架构严格遵循以下解耦原则：

- **Agent Core (Runtime) 与 Tool 分离**: `Runtime` 通过工具注册表与工具交互，不依赖任何具体工具的实现。
- **Tool 与 Agent 分离**: 工具被设计为独立的、可自验证的单元，不依赖于 Agent 的内部状态。
- **核心组件独立**: Memory, Context, Trace, Error 等核心服务都是独立的模块，有清晰的 API，可以被 `Runtime` 或其他组件调用。
- **模型可替换**: `Model Client` 的设计使得底层大语言模型可以轻松更换。
